...
export default async function CheckoutPage({ params, searchParams }: { params: Promise<{beatId:string;licenseCode:string}>; searchParams: Promise<Record<string,string|string[]|undefined>> }) {
  ...
  return <div className="portal-shell narrow-shell"><span className="eyebrow">SECURE CHECKOUT</span><h1>{beat.title}</h1><div className="checkout-summary"><div><strong>{license.name}</strong><p>{license.short_description}</p></div><strong>{formatPrice(price)}</strong></div>{query.cancelled&&<p className="notice">Payment cancelled. Nothing was charged.</p>}<CheckoutForm beatId={beatId} licenseCode={licenseCode} disabled={!hasStripe||license.is_exclusive}/><p className="fine-print">Cards, Apple Pay and bank transfer are processed by Stripe. Files unlock only after a verified webhook confirms payment.</p></div>;
}
